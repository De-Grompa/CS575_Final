...
namespace CS575_Final.Controllers
{
    ...
    public class MoviesController : ControllerBase
    {
        ...
        {
            var movies = _context.Movies?.Include(m => m.genres).Include(m => m.director).ToList();
            if (movies == null)
            {
                return NotFound();
            }
        ...
        {
            var movie = _context.Movies?.Include(m => m.genres).Include(m => m.director).FirstOrDefault(m => m.id == id);
            if (movie == null)
            {
                return NotFound();
            }
